...
func TestWritePlanText(t *testing.T) {
	...
	actual := inner.String()
	if actual != expected {
		t.Errorf("GetDetails %q, want %q", actual, expected)
	}
}
...
func TestWriteParseText(t *testing.T) {
	...
	if actualTail != expectedTail {
		t.Errorf("GetDetails %q, want %q", actualTail, expectedTail)
	}
	...
	if actualHead != expectedHead {
		t.Errorf("GetDetails %q, want %q", actualHead, expectedHead)
	}
	...
	if actualBothEnds != expectedBothEnds {
		t.Errorf("GetDetails %q, want %q", actualBothEnds, expectedBothEnds)
	}
	...
	if actualManyEsc != expectedManyEsc {
		t.Errorf("GetDetails %q, want %q", actualManyEsc, expectedManyEsc)
	}
	...
	if actualSplit != expectedSplit {
		t.Errorf("GetDetails %q, want %q", actualSplit, expectedSplit)
	}
}
...
func TestWriteAnsiColorText(t *testing.T) {
	...
	assertTextAttribute := func(expectedText string, expectedAttributes uint16, ansiColor string) {
		...
		if actualText != expectedText {
			t.Errorf("GetDetails %q, want %q", actualText, expectedText)
		}
		if err != nil {
			t.Fatal("Could not get ConsoleScreenBufferInfo")
		}
		if actualAttributes != expectedAttributes {
			t.Errorf("Text: %q, GetDetails 0x%04x, want 0x%04x", expectedText, actualAttributes, expectedAttributes)
		}
	}
