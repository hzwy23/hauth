...
func init() {
	...
	if "oracle" == defdb {
						...
						select * from (
							select distinct t.res_id,
							  ...
							  from (select *
									  ...
									  start with res_up_id = :1 connect by prior res_id = res_up_id
									 ) t
							 inner join sys_theme_value v
								on t.res_id = v.res_id
							 inner join sys_user_theme h
								on v.theme_id = h.theme_id
							 inner join sys_role_user_relation r
								on r.user_id = h.user_id
							 inner join sys_role_resource_relat e
								on r.role_id = e.role_id
							   and e.res_id = t.res_id
							 inner join sys_role_info f
								on f.role_id = e.role_id
							   and f.role_status_id = '0'
							 where v.res_type = :2
							   and h.user_id = :3
							) rst order by  rst.sort_id`
		...
		sys_rdbms_091 = `select t.code_number,t.role_name,t.role_owner,t.role_create_date,a.role_status_desc,a.role_status_id,t.domain_id,o.domain_name,t.role_maintance_date,t.role_maintance_user,t.role_id from sys_role_info t inner join sys_role_status_attr a on t.role_status_id = a.role_status_id inner join sys_domain_info o on t.domain_id = o.domain_id where t.role_id = :1`
		sys_rdbms_092 = `select distinct t.res_id,t.res_name,t.res_up_id from sys_resource_info t inner join sys_role_resource_relat r on t.res_id = r.res_id where r.role_id = :1`
